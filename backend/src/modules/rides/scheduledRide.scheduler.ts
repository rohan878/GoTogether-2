...
// ✅ requirement: 1 hour before
const WINDOW_MINUTES = Number(process.env.SCHEDULE_REMINDER_WINDOW_MINUTES || 60);
// tolerance window to avoid missing exact minute
...
export async function runScheduledRideReminderOnce() {
  ...
  // We want scheduledFor ≈ now + WINDOW_MINUTES (within tolerance)
...
export function startScheduledRideReminderScheduler() {
  if (started) return;
  started = true;
  ...
  setInterval(async () => {
    try {
      ...
      console.error("scheduled ride scheduler error:", e);
    }
  }, 60_000);

  console.log("✅ Scheduled ride reminder scheduler started (in-app, 1 hour before)");
}
